...
<body>
  ...
  <footer class="footer">
    ...
    // 9. Tag-based article listing page: /articles/tag/<TAG>
    if (url.pathname.startsWith('/articles/tag/') && method === 'GET') {
      const targetTag = decodeURIComponent(url.pathname.replace('/articles/tag/', '')).trim();
      ...
      const filtered = feed.filter((a: any) =>
        a.tags && Array.isArray(a.tags) && a.tags.some((t: string) => t.toLowerCase() === targetTag.toLowerCase() || t.toLowerCase().includes(targetTag.toLowerCase()))
      );
...
<head>
  ...
  <style>
    ...
    .article-hero-img { width: 100%; height: 160px; object-fit: cover; border-radius: 6px; margin-bottom: 14px; }
    .article-card h2 { color: #003f2d; margin: 0 0 12px; font-size: 1.3rem; }
    .article-card p { color: #555; font-size: 0.95rem; line-height: 1.5; margin-bottom: 12px; }
    .tag-badge {
      background: #e6f4ef;
      color: #007f50;
      font-size: 0.75rem;
...
<body>
  ...
  <div class="tag-hero">
    <h1>Articles Tagged "${targetTag}"</h1>
    <p>Showing ${filtered.length} article${filtered.length === 1 ? '' : 's'} topic-matched to ${targetTag}</p>
  </div>
    ...
    ${filtered.length > 0 ? filtered.map((a: any) => {
      ...
      return `
        <div class="article-card">
          ${a.hero ? `<img src="${a.hero}" alt="${a.title}" class="article-hero-img">` : ''}
          <h2>${a.title}</h2>
          <p>${a.summary}</p>
          <div style="margin-bottom:14px;">
            ${(a.tags || []).map((t: string) => `<a href="/articles/tag/${encodeURIComponent(t)}" class="tag-badge">${t}</a>`).join('')}
