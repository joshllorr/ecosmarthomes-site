    ...
    if ((url.pathname === "/api/articles-feed" || url.pathname === "/api/articles-search") && method === "GET") {
      try {
        ...
        try {
          ...
          console.error("KV read articles_feed_index notice:", kvReadErr);
        }
        ...
        console.error("Critical articles-feed error:", feedErr);
        return new Response(JSON.stringify({
          items: [],
          total: 0,
          page: 1,
          pageSize: 6,
          totalPages: 1,
          error: feedErr?.message || "Internal feed error"
        }), {
          status: 200,
          headers: { "Content-Type": "application/json; charset=utf-8", ...corsHeaders }
        });
...
${unique.map(u => `  <url>\n    <loc>${u}</loc>\n  </url>`).join("\n")}
  ...
  <body>
    ...
    // 7. Articles Feed Endpoint handled upstream at line 488 via unified SEO Hub bridge


    // 11. Article Full-Text Search Endpoint handled upstream at line 488 via unified SEO Hub bridge
